...
TH1F* histo = new TH1F("histo", "Histogram", 11, 87, 141);
...
histo->SetTitle("Multi-fitting Example");
histo->GetXaxis()->SetTitle("Counts");
histo->GetYaxis()->SetTitle("Number of Counts");

/* TF1* poisson_fit = new TF1("poisson_fit", "[0]*TMath::Poisson(x,[1])", 70, 160);

  poisson_fit->SetParameters(500, 110);
  // Cosmetics
...
TF1* gauss_fit = new TF1("gauss_fit", "[0]*TMath::Gaus(x,[1],[2]", 80, 150);
gauss_fit->SetParameters(500, 110, 10);
// Cosmetics
