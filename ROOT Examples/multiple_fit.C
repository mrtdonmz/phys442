// Example multi-fitting macro written for ROOT
{
   ...
   TGraphErrors *mygraph = new TGraphErrors("multiple_fit_data.txt");
   ...
   TF1* expo_draw = new TF1("expo_draw", "[0]*exp([1]*x)", 175, 250);
   expo_draw->SetParameters(100, -0.05);
   expo_draw->SetLineColor(kBlack);
   expo_draw->SetLineWidth(5);
	mygraph->Fit(expo_draw, "R+");
